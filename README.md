...
runState 是一个用 Go 和 PostgreSQL 实现的固定步骤 durable task runtime。当前第一切片支持通过 HTTP 创建与查询立即执行的 `echo` / `sleep` 步骤，多个 Worker 以数据库租约竞争任务，并在进程崩溃后接管未完成步骤。
...
`run_at`、审批步骤以及其控制接口属于后续 ticket；当前创建请求会明确拒绝这些定义。
...
完整测试会先等待数据库健康、编译真实 Worker，然后覆盖 HTTP 行为、并发 Claim、事务回滚、租约过期、旧 Worker fencing 和真实进程崩溃接管：
