...
runState 是一个用 Go 和 PostgreSQL 实现的固定步骤 durable task runtime。当前支持通过 HTTP 创建与查询立即或定时执行的 `echo` / `sleep` / `flaky` / `fake_tool` / `approval` 步骤，多个 Worker 以数据库租约竞争任务，并由独立 Scheduler 持久唤醒定时任务、重试及等待超时的任务。
...
创建请求可选 `run_at`，格式为带明确时区偏移的 RFC3339 时间戳，例如 `2026-09-22T10:00:00+08:00`。省略时按数据库创建时间立即排队；指定未来时间时创建为 `SCHEDULED`，到期后 Scheduler 原子转换为 `RUNNABLE` 并记录事件；指定已到期时间则直接创建为 `RUNNABLE`。`run_at` 表示一个固定时间点，不支持 Cron、周期、日历时区或 misfire 策略。首次排队不消耗 Task timeout，Worker 首次 Claim 时才设置 deadline；之后 retry 使用独立的 `retry_at`。

取消 `SCHEDULED`、`RUNNABLE`、`RETRY_WAIT` 等非运行任务会在请求事务内结束为 `CANCELLED` 并返回 200；尚未唤醒的定时任务不会再被 Scheduler 执行。取消 `RUNNING` 任务只先持久化请求并返回 202，Worker 或 Scheduler 随后完成终态转换。重复取消已取消任务返回 200，重复请求仍在运行的任务返回 202，取消其他终态返回 409。202 只表示请求已持久化，不表示工具已经停止。
...
Scheduler 默认每秒扫描一次，可通过 `RUNSTATE_SCHEDULER_POLL_INTERVAL` 调整；可同时运行多个实例。任务、重试和 deadline 都由 PostgreSQL 时间裁决，因此 Scheduler 重启不会丢失待处理的定时任务。
